...
module.exports = async function (props) {
  ...
  app.get("/metrics", async (_, res) => {
    ...
    const mediasoup_producers_nack_packet_count = producersStats.map(
      (stats) => ({
        labels: {
          kind: stats.kind,
          mimeType: stats.mimeType,
    ...
    const mediasoup_producers_packets_discarded = producersStats.map(
      (stats) => ({
        labels: {
          kind: stats.kind,
          mimeType: stats.mimeType,
    ...
    const mediasoup_producers_packets_repaired = producersStats.map(
      (stats) => ({
        labels: {
          kind: stats.kind,
          mimeType: stats.mimeType,
    ...
    const mediasoup_producers_packets_retransmitted = producersStats.map(
      (stats) => ({
        labels: {
          kind: stats.kind,
          mimeType: stats.mimeType,
    ...
    const mediasoup_consumers_nack_packet_count = consumersStats.map(
      (stats) => ({
        labels: {
          kind: stats.kind,
          mimeType: stats.mimeType,
    ...
    const mediasoup_consumers_packets_discarded = consumersStats.map(
      (stats) => ({
        labels: {
          kind: stats.kind,
          mimeType: stats.mimeType,
    ...
    const mediasoup_consumers_packets_repaired = consumersStats.map(
      (stats) => ({
        labels: {
          kind: stats.kind,
          mimeType: stats.mimeType,
    ...
    const mediasoup_consumers_packets_retransmitted = consumersStats.map(
      (stats) => ({
        labels: {
          kind: stats.kind,
          mimeType: stats.mimeType,
    ...
    let mediasoup_trasports_recvBitrate = {
      labels: {
        kind: "recvBitrate",
      },
      value: 0,
    ...
    Object.entries(obj).forEach(([key, o]) => {
      if (Array.isArray(o)) {
        const gauge = new promClient.Gauge({
          ...
          help: key,
          labelNames:
            typeof o[0] === "object" && typeof o[0].labels === "object"
